...
class ASTGenerator:
    ...
    def traverse(self, node):
        ...
        json_node = {"id": pos}
        self._nodes.append(json_node)
        json_node["type"] = "py_" + type(node).__name__
        children = []
        if isinstance(node, ast.Name):
            ...
            if node.value is True:
                json_node["value"] = "True"
            elif node.value is False:
                json_node["value"] = "False"
            elif node.value is None:
                json_node["value"] = "None"
            else:
                pass
        ...
        elif isinstance(node, (ast.BinOp, ast.BoolOp, ast.Compare)):
            json_node["type"] = "BinaryOperation"
        elif isinstance(node, ast.UnaryOp):
            json_node["type"] = "UnaryOperation"
